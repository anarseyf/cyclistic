...
wide %>%
  ...
  footnote(
    general_title = "",
    general = "(In this table and in Table 3, the larger of the two numbers for each month is in italics.)"
  )
```
...
# map13 <- get_map(location = "Chicago", zoom = 13, maptype = "toner-lite")
...
### 1. Members use the service as a utility, Casual riders as more of a luxury
...
Make it easy for casual users to consider Cyclistic as a regular transportation option. To make this effortless, consider removing the barrier to entry and making the experience frictionless. For example, automatically turn a single pass into a free weekly or monthly trial of the membership, send out a promotion for such a trial, or advertise specific days when these trials will be offered.
