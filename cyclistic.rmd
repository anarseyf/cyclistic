...
```{r, eval=TRUE}
text <- "The Pearson correlation coefficient measures how strongly two sets of data are linearly correlated, from -1 (perfect negative) to 1 (perfect positive), with 0 indicating no correlation. Here, ride count is negatively correlated with snow: the more snow, the fewer rides."

weather_correlations %>%
  kable_custom(caption = "Correlation (r) between weather and number of rides") %>%
  footnote(general = text, threeparttable = TRUE, general_title = " ")
```

Table 5 shows that the number of rides on a given day is strongly—almost perfectly—correlated with **air temperature** for both groups. The following plots help illustrate this relationship. Each circle represents one day; 365 circles for each plot; the diagonal lines represent best linear fit and confidence intervals.
...
**Rain** is more ambiguous: correlation is close to 0. Does this mean bike-share users in Chicago are not influenced by rain at all? The more plausible explanation is that we are at the limits of this dataset. In the 12-month period, 256 days had no recorded rain, only 16 days had half an inch or more; on those days, Members rode for 4% less total time, and Casual users 6% more, than on days with less rain. In other words, it did not rain enough to make the relationship clear.

The relationship with **Snow** is clearer, and snow itself is obviously not independent from temperature.

These distributions confirms an observation from section 1: average ride duration remains nearly flat (more so for Members), but ride volume varies significantly.

(Note: one outlier for Casual riders, 16 Feb 2021, was removed from the duration scatterplot as it had an average duration of over 100 minutes. More on this in the cleanup notes in the Appendix.)
