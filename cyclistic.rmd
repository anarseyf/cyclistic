...
packages <- c(
  "tidyr", "knitr", "ggplot2", "ggmap", "gridExtra",
  "maps", "tibble", "ggnewscale", "kableExtra", "tinytex"
)
...
agg_hourly %>%
  ...
  # scale_fill_gradientn(colors = heat.colors(20)) +
  scale_fill_gradient(
    low = "#600000", high = "yellow",
...
agg_weekly_weather %>%
  ...
  geom_step(aes(
    y = avg_count, group = is_member,
    color = is_member, size = 0.5
  ), stat = "identity") +
  geom_point(aes(y = 0, size = total_snow / 10),
    color = "#e30f84", alpha = 0.7
  ) +
...
p2 <- scatterplot %>%
  ...
  facet_wrap(~IS_MEMBER) +
  labs(
    title = "Correlation: temp vs ride duration",
  ) +
  guides(color = "none")
